...
//   const [activityTypes, setActivityTypes] = useState<Report['activityType'][]>([]);
...
//     } else {
//       setAvailableSmallGroups(mockSmallGroups);
//     }
//     setValue("smallGroupId", undefined); // Reset small group selection when site changes
...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { useToast } from "@/hooks/use-toast";
import type { Report, ActivityType } from "@/lib/types";
import { UploadCloud, FileText, Send, CalendarIcon } from "lucide-react";
import Image from "next/image";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Calendar } from "@/components/ui/calendar";
...
export function ReportForm({ onSubmitSuccess }: ReportFormProps) {
  ...
  const processSubmit = async (data: ReportFormData) => {
    ...
    const submittedReport: Omit<Report, 'status'> = {
      id: `rep_${Date.now()}`,
      submittedBy: "current_user_id_mock",
      submissionDate: new Date().toISOString(),
      ...data,
      activityDate: data.activityDate.toISOString(),
      participantsCountReported,
    ...
    console.log("Report Submitted:", finalReport);
    toast({
      title: "Report Submitted Successfully!",
      description: `Your report "${data.title}" has been recorded.`,
      variant: "default",
    ...
    reset();
    setSelectedFiles([]);
    if (onSubmitSuccess) onSubmitSuccess(finalReport);
    setIsSubmitting(false);
  };
      ...
      <CardContent>
        <form onSubmit={handleSubmit(processSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <Label htmlFor="activityTypeId">Type of Activity</Label>
              <Controller
                name="activityTypeId"
                control={control}
                render={({ field }) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input id="expenses" type="number" step="0.01" {...register("expenses")} placeholder="0.00" className="mt-1" />
              {errors.expenses && <p className="text-sm text-destructive mt-1">{errors.expenses.message}</p>}
            </div>
            <div>
              <Label htmlFor="currency">Currency (Optional)</Label>
               <Controller
          ...
          <div>
            <Label htmlFor="financialSummary">Additional Financial Notes (Optional)</Label>
            <Textarea id="financialSummary" {...register("financialSummary")} placeholder="Brief overview of income, expenses, or donations if not covered by 'Expenses'." rows={3} className="mt-1" />
            {errors.financialSummary && <p className="text-sm text-destructive mt-1">{errors.financialSummary.message}</p>}
          </div>
