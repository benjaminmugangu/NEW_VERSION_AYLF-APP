 ...
 */
export async function updateProfile(userId: string, updates: Partial<User>, ipAddress?: string, userAgent?: string): Promise<ServiceResponse<User>> {
  try {
    if ('role' in updates) {
    ...
    const result = await withRLS(currentUserSession.id, async () => {
      return await basePrisma.$transaction(async (tx: any) => {
        ...
        const data = await tx.profile.update({
          where: { id: userId },
          data: dbUpdates,
          include: {
            site: true,
        ...
        const updatedUser = await mapProfileToUser(data);
 ...
 */
export async function deleteUser(userId: string, ipAddress?: string, userAgent?: string): Promise<ServiceResponse<void>> {
  try {
    const { getUser } = getKindeServerSession();
    ...
    await withRLS(currentUser.id, async () => {
      return await basePrisma.$transaction(async (tx: any) => {
        ...
        const requester = await tx.profile.findUnique({
        ...
        await createAuditLog({
          ...
          entityId: userId,
          metadata: { before, entity: 'Profile' },
          ipAddress,
          userAgent
        }, tx);
